...
ForwardIt2 parallel_transform(int concurrencyLevel,
                              ForwardIt1 first1,
                              ForwardIt1 last1,
                              ForwardIt2 first2,
                              UnaryOp &&unary_op)
{
    ...
    else
    {
        ...
        for (size_t i = 0; i < concurrencyLevel; i++)
        {
            ...
            std::thread th([begin, last1_temp, first2_temp, &unary_op]() {
                std::transform(begin, last1_temp, first2_temp, std::forward<UnaryOp>(unary_op));
            });
...
int main()
{
    ...
    std::generate(std::begin(vec1), std::end(vec1), []() {
        static int i{};
        return i++;
    });
    ...
    std::generate(std::begin(vec1Prime), std::end(vec1Prime), []() {
        static int i{};
        return i++;
    });
    t1 = std::chrono::system_clock::now();
    parallel_transform(cores, std::begin(vec1Prime), std::end(vec1Prime), std::begin(vec2Prime), [](int &i) {
        return i * 2;
    });
    t2 = std::chrono::system_clock::now();
    auto tSingleCore = t2 - t1;
    std::cout << "Execution time for parallel processing of " << ParallelQuantity << " data is " << tParallel.count()
              << std::endl;
    std::cout << "Execution time for parallel processing of " << singleCoreQuant << " data is " << tParallel.count()
              << std::endl;
    return 0;
}
