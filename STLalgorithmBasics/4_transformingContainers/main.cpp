...
int main()
{
    ...
    std::transform(std::begin(vec), std::end(vec), std::ostream_iterator<int> {std::cout , ", "}, [](int i) ->int {return i*i;});
    std::cout << '\n';
    auto int_to_string([](int i){return std::to_string(i);});
    auto int_to_string2([](int i){std::stringstream ss;ss << i << "^2 = " << i * i << '\n'; return ss.str();});
